    ...
    loop {
                ...
                if !s.track_name.is_empty() {
                    if let Some(map) = TrackMap::load(&tracks_dir, &s.track_name, s.track_length) {
                        ...
                        if centerline.is_none() {
                            centerline =
                                centerline::Centerline::load(&tracks_dir, &map.file_stem());
        ...
        for point in &new_points {
                        ...
                        if let Some(c) = map.corner_by_id(prev).cloned() {
                            ...
                            if let Some(best) = tips.into_iter().max_by_key(|t| t.priority) {
                                ...
                                if streak >= 5 {
                                    ...
                                    pending_corner_tips
                                        .insert(prev, (text.clone(), priority, time_loss));
                                    if first_time {
                                        send_tip_text(
                                            text,
                                            Some(prev),
            ...
            if let Some(lap) = completed {
                ...
                let current_corner_count = track_map.as_ref().map(|m| m.corners.len()).unwrap_or(0);
                ...
                if lap.samples.len() >= MIN_DETECTION_SAMPLES {
                    ...
                    if want_detect {
                        let corners = CornerDetector::detect(centerline.as_ref(), &lap.samples);
        ...
        if last_status_at.elapsed() >= STATUS_INTERVAL {
            ...
            let status = CoachStatus {
                ...
                map_corners: track_map
                    ...
                    .unwrap_or_default(),
                map_track_length_m: track_map.as_ref().map(|m| m.track_length_m).unwrap_or(0.0),
            };
